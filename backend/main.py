...
async def import_employees(file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    Поддерживает .xlsx и .csv с устойчивым парсингом.
    """
    try:
        ...
        # 🔹 Парсинг Excel с обработкой ошибок openpyxl
        if filename_lower.endswith('.xlsx'):
            try:
                # Первая попытка: стандартный openpyxl
                df = pd.read_excel(io.BytesIO(contents), engine='openpyxl')
            except ValueError as e:
                if "stylesheet" in str(e).lower() or "xml" in str(e).lower():
                    # 🔹 Вторая попытка: игнорировать стили
                    logger.warning("⚠️ openpyxl не смог прочитать стили, пробуем с data_only=True")
                    try:
                        df = pd.read_excel(
                            io.BytesIO(contents), 
                            engine='openpyxl',
                        ...
                        # 🔹 Третья попытка: использовать calamine (если установлен)
                        try:
                            df = pd.read_excel(io.BytesIO(contents), engine='calamine')
                            logger.info("✅ Файл прочитан через calamine")
                        except ImportError:
                            logger.error("❌ Установите fastexcel для лучшей совместимости: pip install fastexcel")
                            raise HTTPException(
                                400, 
                                detail="Файл Excel содержит невалидные стили. "
                                       "Сохраните его заново в Excel или установите пакет fastexcel"
                            )
                ...
                # openpyxl не установлен — пробуем calamine
                try:
                    df = pd.read_excel(io.BytesIO(contents), engine='calamine')
                except ImportError:
                    raise HTTPException(
                        400, 
                        detail="Не установлен движок для чтения Excel. Установите: pip install openpyxl"
                    )
        ...
        # Нормализация названий колонок (убираем пробелы, нижний регистр)
        df.columns = df.columns.str.strip().str.lower()
        
        # Маппинг колонок с поддержкой вариантов написания
        col_map = {}
        ...
        if len(col_map) != 3:
            raise HTTPException(
                400, 
                detail=f"Файл должен содержать колонки: ФИО, Центр, Подразделение. "
                       f"Найдено: {list(df.columns)}. Сопоставлено: {col_map}"
            )
        ...
        # Формируем множество уникальных ключей
        file_keys = set()
...
def quarterly_report(quarter: str = Query(...), db: Session = Depends(get_db)):
    ...
    Сотрудник считается «прошедшим проверку», если у него есть хотя бы одна сессия в этом квартале.
    """
    ...
    centers_list = sorted(centers_dict.values(), key=lambda x: x["center"])
    return {
        "quarter": quarter,
        "centers": centers_list
    }
